// PulseForge — Market Macro Intelligence Dashboard
// Loads data from /data/*.json (generated by GitHub Actions pipeline)
// OR from Google Apps Script if APPS_SCRIPT_URL is configured.
//
// v2: Added Ticker Tape, Market News, Sector Heatmap, Insider Flow, Congress Watch

// ── Google Sheets Config ──────────────────────────────────────────────────
// To use Google Sheets as the data backend:
//   1. Deploy PulseForge/appsscript/Code.gs as a Web App in Google Apps Script UI
//   2. Copy the deployment URL and paste it below
//   3. Set access to "Anyone" when deploying
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyVGZRO4XVS0UPhzVRYJJH-_LrAHAAiBkiYo9Ko1qJk_JlaITcCyvITMDxzVEEKTzkd/exec';
const USE_SHEETS = APPS_SCRIPT_URL !== '';

// ── Finnhub Client-Side Key (free tier) ──────────────────────────────────
const FINNHUB_KEY = 'd6952r9r01qs7u9kq240d6952r9r01qs7u9kq24g';
const FINNHUB_BASE = 'https://finnhub.io/api/v1';

// Mega-caps for ticker tape (beyond watchlist)
const TICKER_TAPE_SYMBOLS = ['SPY','QQQ','TSLA','NVDA','AAPL','MSFT','META','AMZN','GOOGL','PLTR','HOOD','SOFI','RIVN','NIO','JPM','BTC-USD'];

const DATA_BASE = 'data';
const LS_KEY = 'pf_watchlist_v2';
const DEFAULT_TICKERS = ['TSLA', 'PLTR', 'AMZN', 'HOOD', 'SOFI', 'RIVN', 'NIO'];

const PLOTLY_DARK = {
    paper_bgcolor: 'rgba(0,0,0,0)',
    plot_bgcolor: '#ffffff',
    font: { family: "'Source Serif 4', Georgia, serif", color: '#333333', size: 12 },
    xaxis: { gridcolor: '#e5e5e5', linecolor: '#cccccc', zerolinecolor: '#cccccc' },
    yaxis: { gridcolor: '#e5e5e5', linecolor: '#cccccc', zerolinecolor: '#cccccc' },
    margin: { l: 50, r: 20, t: 30, b: 40 },
    hoverlabel: { bgcolor: '#000000', bordercolor: '#C41E3A', font: { color: '#ffffff' } }
};
const PLOTLY_CONFIG = { responsive: true, displayModeBar: false };

// ── Utilities ──
function fmt(n, decimals = 2) {
    if (n == null) return '--';
    return Number(n).toLocaleString('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
}

function fmtChange(current, previous) {
    if (current == null || previous == null) return { text: '--', cls: 'flat' };
    const diff = current - previous;
    const pct = ((diff / Math.abs(previous)) * 100).toFixed(2);
    const sign = diff >= 0 ? '+' : '';
    const cls = diff > 0 ? 'up' : diff < 0 ? 'down' : 'flat';
    return { text: `${sign}${pct}%`, cls };
}

function regimeColor(score) {
    if (score < 30) return { label: 'EXTREME FEAR — Markets in distress, capitulation risk elevated.', cls: 'fear', color: '#C41E3A' };
    if (score < 45) return { label: 'FEAR — Risk-off sentiment prevailing, defensive positioning warranted.', cls: 'fear', color: '#C41E3A' };
    if (score < 55) return { label: 'NEUTRAL — Mixed signals. No directional conviction.', cls: 'neutral', color: '#666666' };
    if (score < 70) return { label: 'GREED — Momentum building. Risk appetite elevated.', cls: 'greed', color: '#1a6b1a' };
    return { label: 'EXTREME GREED — Euphoria detected. Caution advised.', cls: 'greed', color: '#1a6b1a' };
}

async function loadJSON(filename) {
    try {
        const resp = await fetch(`${DATA_BASE}/${filename}`);
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        return await resp.json();
    } catch (e) {
        console.warn(`Failed to load ${filename}:`, e.message);
        return null;
    }
}

// ── Google Sheets Loader ──────────────────────────────────────────────────
/**
 * Fetch all data from Google Apps Script endpoint.
 * Returns an object { metrics, pulse, sectors, watchlist, vol, predictions, macro }
 * matching the format expected by the render functions.
 */
async function loadFromSheets() {
    try {
        console.log('[PulseForge] Loading data from Google Sheets...');
        const resp = await fetch(APPS_SCRIPT_URL);
        if (!resp.ok) throw new Error(`Apps Script HTTP ${resp.status}`);
        const d = await resp.json();
        if (d.error) throw new Error(`Apps Script error: ${d.error}`);

        // Map Apps Script response → existing data formats
        // pulse: already in { dates, scores, signals, current_score, last_updated }
        const pulse = d.pulse || null;

        // sectors: Apps Script returns array of {name, symbol, price, change_pct}
        // Old format: { last_updated, sectors: { "Technology": { symbol, price, change_pct } } }
        const sectors = d.sectors && d.sectors.length > 0 ? {
            last_updated: d.last_updated,
            sectors: Object.fromEntries(d.sectors.map(s => [
                s.name, { symbol: s.symbol, price: s.price, change_pct: s.change_pct }
            ]))
        } : null;

        // watchlist: Apps Script returns array of {ticker, price, change_pct, volume, signal, notes}
        // Old format: { last_updated, stocks: [...] }
        const watchlist = d.watchlist && d.watchlist.length > 0 ? {
            last_updated: d.last_updated,
            stocks: d.watchlist
        } : null;

        // volatility: Apps Script returns { vix_history: {dates, values}, vix_sma: {dates, values} }
        const vol = d.volatility || null;

        // predictions: Apps Script returns array of {name, direction, confidence, horizon, rationale}
        // Old format: { last_updated, predictions: [...] }
        const predictions = d.predictions && d.predictions.length > 0 ? {
            last_updated: d.last_updated,
            predictions: d.predictions.map(p => ({
                name: p.name,
                direction: p.direction,
                confidence: p.confidence,
                horizon: p.horizon,
                rationale: p.rationale
            }))
        } : null;

        // macro: Apps Script returns { notes: [...] }
        const macro = d.macro || null;

        // metrics: not stored in Sheets — fall back to JSON file
        const metrics = await loadJSON('metrics.json');

        // Extract the real pipeline push timestamp (stored in the Predictions timestamp column).
        // This is when data was *written* to Sheets, not when the page fetched it.
        let data_pushed_at = null;
        if (d.predictions && d.predictions.length > 0 && d.predictions[0].timestamp) {
            data_pushed_at = String(d.predictions[0].timestamp);
        } else if (d.pulse && d.pulse.dates && d.pulse.dates.length > 0) {
            data_pushed_at = d.pulse.dates[d.pulse.dates.length - 1]; // date-only fallback
        }

        console.log('[PulseForge] Sheets data loaded successfully. Push time:', data_pushed_at);
        return { metrics, pulse, sectors, watchlist, vol, predictions, macro, data_pushed_at, source: 'sheets' };
    } catch (e) {
        console.warn('[PulseForge] Sheets load failed, falling back to JSON files:', e.message);
        return null;
    }
}

// ── Watchlist Management ──
function getWatchlist() {
    try {
        const saved = localStorage.getItem(LS_KEY);
        if (saved) return JSON.parse(saved);
    } catch (e) {}
    return [...DEFAULT_TICKERS];
}

function saveWatchlist(tickers) {
    localStorage.setItem(LS_KEY, JSON.stringify(tickers));
}

function addTicker() {
    const input = document.getElementById('tickerInput');
    const ticker = input.value.trim().toUpperCase().replace(/[^A-Z]/g, '');
    if (!ticker) return;
    const list = getWatchlist();
    if (list.includes(ticker)) { input.value = ''; return; }
    list.push(ticker);
    saveWatchlist(list);
    input.value = '';
    renderChartsTab();
}

function removeTicker(ticker) {
    const list = getWatchlist().filter(t => t !== ticker);
    saveWatchlist(list);
    renderChartsTab();
}

// ── Tab Switching ──
const TAB_LOADED = {};
function switchTab(name, btn) {
    document.querySelectorAll('.section-nav-item').forEach(b => b.classList.remove('active'));
    document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
    if (btn) btn.classList.add('active');
    document.getElementById('tab-' + name).classList.add('active');
    if (name === 'charts') renderChartsTab();
    if (name === 'news'     && !TAB_LOADED.news)     { loadNewsTab();    TAB_LOADED.news = true;     }
    if (name === 'insider'  && !TAB_LOADED.insider)  { renderInsiderTab(); TAB_LOADED.insider = true;  }
    if (name === 'congress' && !TAB_LOADED.congress) { renderCongressTab(); TAB_LOADED.congress = true; }
}

// ── Sparkline ──
function renderSparkline(elementId, values, color = '#000000') {
    const el = document.getElementById(elementId);
    if (!el || !values || values.length < 2) return;
    Plotly.newPlot(el, [{
        y: values.slice(-30), type: 'scatter', mode: 'lines',
        line: { color, width: 1.5, shape: 'spline' },
        fill: 'tozeroy',
        fillcolor: color === '#C41E3A' ? 'rgba(196,30,58,0.08)' : 'rgba(0,0,0,0.06)',
        hoverinfo: 'skip'
    }], {
        ...PLOTLY_DARK,
        margin: { l: 0, r: 0, t: 0, b: 0 },
        xaxis: { visible: false }, yaxis: { visible: false }, height: 36
    }, { ...PLOTLY_CONFIG, staticPlot: true });
}

// ── Overview Renders ──
function renderMetrics(data) {
    if (!data) return;
    const metrics = [
        { id: 'vix', key: 'VIX', color: '#C41E3A', decimals: 2, prefix: '', suffix: '' },
        { id: 'spy', key: 'SPY', color: '#000000', decimals: 2, prefix: '$', suffix: '' },
        { id: 'dxy', key: 'DXY', color: '#333333', decimals: 2, prefix: '', suffix: '' },
        { id: 'tny', key: 'TNX', color: '#333333', decimals: 2, prefix: '', suffix: '%' },
        { id: 'btc', key: 'BTC', color: '#C41E3A', decimals: 0, prefix: '$', suffix: '' },
        { id: 'oil', key: 'CL', color: '#333333', decimals: 2, prefix: '$', suffix: '' }
    ];
    metrics.forEach(({ id, key, color, decimals, prefix, suffix }) => {
        const series = data[key];
        if (!series) return;
        const values = series.values || [];
        const current = values[values.length - 1];
        const previous = values[values.length - 2];
        document.getElementById(`${id}Value`).textContent = `${prefix}${fmt(current, decimals)}${suffix}`;
        const change = fmtChange(current, previous);
        const changeEl = document.getElementById(`${id}Change`);
        changeEl.textContent = change.text;
        changeEl.className = `change-badge ${change.cls}`;
        renderSparkline(`${id}Spark`, values, color);
    });
    // Update masthead price strip
    const spy = data['SPY']?.values; if (spy) { const v = spy[spy.length-1]; const p = spy[spy.length-2]; const c = fmtChange(v,p); const el = document.getElementById('mastSPY'); if(el) el.textContent = `$${fmt(v,2)} ${c.text}`; el.className = `mono ${c.cls}`; }
    const vix = data['VIX']?.values; if (vix) { const v = vix[vix.length-1]; const el = document.getElementById('mastVIX'); if(el) el.textContent = fmt(v,2); }
    const btc = data['BTC']?.values; if (btc) { const v = btc[btc.length-1]; const el = document.getElementById('mastBTC'); if(el) el.textContent = `$${fmt(v,0)}`; }
}

function renderPulseChart(data) {
    if (!data || !data.dates || !data.scores) return;
    const colors = data.scores.map(s => s < 45 ? '#C41E3A' : s < 55 ? '#888888' : '#1a6b1a');
    Plotly.newPlot('pulseChart', [
        { x: data.dates, y: data.scores, type: 'scatter', mode: 'lines+markers',
          line: { color: '#000000', width: 2, shape: 'spline' },
          marker: { color: colors, size: 5 },
          fill: 'tozeroy', fillcolor: 'rgba(0,0,0,0.05)', name: 'Pulse Score' },
        { x: data.dates, y: Array(data.dates.length).fill(30), type: 'scatter', mode: 'lines',
          line: { color: '#C41E3A', width: 1, dash: 'dot' }, name: 'Fear Zone', hoverinfo: 'skip' },
        { x: data.dates, y: Array(data.dates.length).fill(70), type: 'scatter', mode: 'lines',
          line: { color: '#1a6b1a', width: 1, dash: 'dot' }, name: 'Greed Zone', hoverinfo: 'skip' }
    ], {
        ...PLOTLY_DARK, showlegend: true,
        legend: { orientation: 'h', y: -0.15, font: { size: 11 } },
        yaxis: { ...PLOTLY_DARK.yaxis, range: [0, 100], title: 'Pulse Score' }, height: 380
    }, PLOTLY_CONFIG);

    const latestScore = data.scores[data.scores.length - 1];
    if (latestScore != null) {
        const regime = regimeColor(latestScore);
        document.getElementById('pulseScore').textContent = Math.round(latestScore);
        document.getElementById('pulseScore').style.color = regime.color;
        const regimeEl = document.getElementById('pulseRegime');
        regimeEl.textContent = regime.label;
    }
}

function renderSectorChart(data) {
    if (!data || !data.sectors) return;
    const sectors = data.sectors;
    const names = Object.keys(sectors);
    const values = names.map(n => sectors[n].change_pct);
    const colors = values.map(v => v >= 0 ? '#1a6b1a' : '#C41E3A');
    Plotly.newPlot('sectorChart', [{
        x: names, y: values, type: 'bar',
        marker: { color: colors, line: { color: '#000000', width: 0.5 } },
        text: values.map(v => `${v >= 0 ? '+' : ''}${v.toFixed(2)}%`),
        textposition: 'outside',
        textfont: { family: 'IBM Plex Mono', size: 11, color: '#333333' },
        hovertemplate: '%{x}: %{y:.2f}%<extra></extra>'
    }], {
        ...PLOTLY_DARK,
        yaxis: { ...PLOTLY_DARK.yaxis, title: 'Change %', zeroline: true, zerolinecolor: '#000000', zerolinewidth: 2 },
        height: 350
    }, PLOTLY_CONFIG);
}

function renderWatchlist(data) {
    if (!data || !data.stocks) return;
    const table = document.getElementById('watchlistTable');
    let html = `<table><thead><tr>
        <th>Ticker</th><th>Price</th><th>Change</th><th>Volume</th><th>Signal</th><th>Notes</th>
    </tr></thead><tbody>`;
    data.stocks.forEach(s => {
        const changeCls = s.change_pct >= 0 ? 'up' : 'down';
        const signalCls = s.signal === 'BULLISH' ? 'bullish' : s.signal === 'BEARISH' ? 'bearish' : 'neutral';
        html += `<tr>
            <td class="ticker-cell">${s.ticker}</td>
            <td>$${fmt(s.price)}</td>
            <td class="metric-change ${changeCls}">${s.change_pct >= 0 ? '+' : ''}${fmt(s.change_pct)}%</td>
            <td>${s.volume ? (s.volume / 1e6).toFixed(1) + 'M' : '--'}</td>
            <td><span class="signal ${signalCls}">${s.signal || '--'}</span></td>
            <td style="color:var(--text-muted);font-family:var(--font-sans);font-size:0.8rem;">${s.notes || ''}</td>
        </tr>`;
    });
    html += '</tbody></table>';
    table.innerHTML = html;
}

function renderVolChart(data) {
    if (!data) return;
    const traces = [];
    if (data.vix_history) traces.push({
        x: data.vix_history.dates, y: data.vix_history.values, type: 'scatter', mode: 'lines',
        name: 'VIX', line: { color: '#C41E3A', width: 2 },
        fill: 'tozeroy', fillcolor: 'rgba(196,30,58,0.08)'
    });
    if (data.vix_sma) traces.push({
        x: data.vix_sma.dates, y: data.vix_sma.values, type: 'scatter', mode: 'lines',
        name: 'VIX 20-SMA', line: { color: '#333333', width: 1.5, dash: 'dash' }
    });
    if (traces.length) Plotly.newPlot('volChart', traces, {
        ...PLOTLY_DARK, showlegend: true,
        legend: { orientation: 'h', y: -0.15 },
        yaxis: { ...PLOTLY_DARK.yaxis, title: 'VIX Level' },
        shapes: [
            { type: 'line', y0: 20, y1: 20, x0: 0, x1: 1, xref: 'paper', line: { color: '#666666', width: 1, dash: 'dot' } },
            { type: 'line', y0: 30, y1: 30, x0: 0, x1: 1, xref: 'paper', line: { color: '#C41E3A', width: 1, dash: 'dot' } }
        ],
        height: 350
    }, PLOTLY_CONFIG);
}

function renderPredictions(data) {
    if (!data || !data.predictions) return;
    const grid = document.getElementById('predictionsGrid');
    grid.innerHTML = data.predictions.map(p => {
        const signalCls = p.direction === 'BULLISH' ? 'bullish' : p.direction === 'BEARISH' ? 'bearish' : 'neutral';
        const arrow = p.direction === 'BULLISH' ? '↑' : p.direction === 'BEARISH' ? '↓' : '→';
        return `<div class="prediction-item">
            <div class="prediction-name">${p.name}</div>
            <span class="prediction-signal ${signalCls}">${p.direction} ${arrow}</span>
            <div class="prediction-meta">Confidence: ${p.confidence}% · Horizon: ${p.horizon}</div>
            <div class="prediction-rationale">${p.rationale || ''}</div>
        </div>`;
    }).join('');
}

function renderMacroNotes(data) {
    if (!data || !data.notes) return;
    document.getElementById('macroNotes').innerHTML = '<ul>' + data.notes.map(n => `<li>${n}</li>`).join('') + '</ul>';
}

// ── Live Price Cache (populated by init() from watchlist Sheets data) ──
let livePrices = {}; // ticker -> { price, change_pct }

// ── Lightweight Charts Tab ──
let lwInstances = {}; // ticker -> chart instance (for cleanup)

async function renderChartsTab() {
    const tickers = getWatchlist();
    const grid = document.getElementById('lwChartsGrid');
    const wmTickers = document.getElementById('wmTickers');

    // Render ticker pills
    wmTickers.innerHTML = tickers.map(t =>
        `<span class="wm-pill">
            ${t}
            <button class="wm-remove" onclick="removeTicker('${t}')" title="Remove ${t}">×</button>
        </span>`
    ).join('');

    // Destroy old chart instances
    Object.values(lwInstances).forEach(c => { try { c.remove(); } catch(e){} });
    lwInstances = {};

    // Build chart cards
    grid.innerHTML = tickers.map(t => `
        <div class="lw-card" id="lwcard-${t}">
            <div class="lw-card-header">
                <span class="lw-ticker">${t}</span>
                <span class="lw-price" id="lwprice-${t}">--</span>
                <span class="lw-change" id="lwchg-${t}">--</span>
            </div>
            <div class="lw-chart-wrap" id="lwchart-${t}"></div>
            <div class="lw-vol-wrap" id="lwvol-${t}"></div>
        </div>`
    ).join('');

    // Load candles for each ticker
    for (const ticker of tickers) {
        await renderSingleChart(ticker);
    }
}

async function renderSingleChart(ticker) {
    const data = await loadJSON(`candles/${ticker}.json`);
    const chartEl = document.getElementById(`lwchart-${ticker}`);
    const volEl = document.getElementById(`lwvol-${ticker}`);
    if (!chartEl) return;

    if (!data || !data.candles || data.candles.length === 0) {
        chartEl.innerHTML = `<div class="lw-nodata">No candle data for ${ticker}.<br><span>Will be available after next pipeline run.</span></div>`;
        return;
    }

    const candles = data.candles;
    const last = candles[candles.length - 1];
    const prev = candles[candles.length - 2];

    // Update price/change header — prefer live price from Sheets if available
    const priceEl = document.getElementById(`lwprice-${ticker}`);
    const chgEl   = document.getElementById(`lwchg-${ticker}`);
    const live     = livePrices[ticker];

    if (live && live.price != null) {
        // Show live (intraday) price and today's % change vs last close
        if (priceEl) priceEl.textContent = `$${Number(live.price).toFixed(2)}`;
        if (chgEl) {
            const pct  = live.change_pct != null ? Number(live.change_pct) :
                         ((live.price - last.c) / last.c * 100);
            const sign = pct >= 0 ? '+' : '';
            chgEl.textContent  = `${sign}${pct.toFixed(2)}% today`;
            chgEl.className    = `lw-change ${pct >= 0 ? 'up' : 'down'}`;
        }
    } else {
        // Fallback: use last completed candle
        if (priceEl) priceEl.textContent = `$${last.c.toFixed(2)}`;
        if (chgEl) {
            const chgPct = prev ? ((last.c - prev.c) / prev.c * 100) : 0;
            const sign   = chgPct >= 0 ? '+' : '';
            chgEl.textContent = `${sign}${chgPct.toFixed(2)}%`;
            chgEl.className   = `lw-change ${chgPct >= 0 ? 'up' : 'down'}`;
        }
    }

    // Main candlestick chart
    const chart = LightweightCharts.createChart(chartEl, {
        width: chartEl.clientWidth || 480,
        height: 200,
        layout: { background: { type: 'solid', color: '#ffffff' }, textColor: '#333333' },
        grid: { vertLines: { color: '#f0f0f0' }, horzLines: { color: '#f0f0f0' } },
        crosshair: { mode: LightweightCharts.CrosshairMode.Normal },
        rightPriceScale: { borderColor: '#cccccc' },
        timeScale: { borderColor: '#cccccc', timeVisible: true },
        handleScroll: true,
        handleScale: true,
    });

    const candleSeries = chart.addSeries(LightweightCharts.CandlestickSeries, {
        upColor: '#1a6b1a', downColor: '#C41E3A',
        borderUpColor: '#1a6b1a', borderDownColor: '#C41E3A',
        wickUpColor: '#1a6b1a', wickDownColor: '#C41E3A',
    });

    const ohlcData = candles.map(c => ({
        time: Math.floor(c.t / 1000),
        open: c.o, high: c.h, low: c.l, close: c.c
    }));
    candleSeries.setData(ohlcData);
    chart.timeScale().fitContent();

    // Live price line — dashed horizontal showing where price is RIGHT NOW
    // (pipeline refreshes this every 15 min during market hours)
    const liveNow = livePrices[ticker];
    if (liveNow && liveNow.price != null) {
        const liveP   = Number(liveNow.price);
        const isUp    = liveP >= last.c;
        const lineClr = isUp ? '#10b981' : '#ef4444';
        candleSeries.createPriceLine({
            price: liveP,
            color: lineClr,
            lineWidth: 1,
            lineStyle: LightweightCharts.LineStyle.Dashed,
            axisLabelVisible: true,
            title: `live  $${liveP.toFixed(2)}`,
        });
    }

    lwInstances[ticker] = chart;

    // Volume chart
    const volChart = LightweightCharts.createChart(volEl, {
        width: volEl.clientWidth || 480,
        height: 55,
        layout: { background: { type: 'solid', color: '#ffffff' }, textColor: '#333333' },
        grid: { vertLines: { visible: false }, horzLines: { visible: false } },
        rightPriceScale: { borderColor: '#cccccc', scaleMargins: { top: 0.1, bottom: 0 } },
        timeScale: { borderColor: '#cccccc', visible: false },
        handleScroll: false, handleScale: false,
    });

    const volSeries = volChart.addSeries(LightweightCharts.HistogramSeries, {
        priceFormat: { type: 'volume' },
        priceScaleId: '',
    });

    const volData = candles.map(c => ({
        time: Math.floor(c.t / 1000),
        value: c.v,
        color: c.c >= c.o ? 'rgba(26,107,26,0.5)' : 'rgba(196,30,58,0.5)'
    }));
    volSeries.setData(volData);
    volChart.timeScale().fitContent();

    // Sync time scales
    chart.timeScale().subscribeVisibleLogicalRangeChange(range => {
        if (range) volChart.timeScale().setVisibleLogicalRange(range);
    });
    volChart.timeScale().subscribeVisibleLogicalRangeChange(range => {
        if (range) chart.timeScale().setVisibleLogicalRange(range);
    });

    // Resize observer
    const observer = new ResizeObserver(() => {
        const w = chartEl.clientWidth;
        if (w > 0) { chart.resize(w, 200); volChart.resize(w, 55); }
    });
    observer.observe(chartEl);
}

// ── Data Source Status ──
function setDataStatus(connected, pushedAt) {
    const dot   = document.getElementById('dsDot');
    const label = document.getElementById('dsLabel');
    const time  = document.getElementById('lastUpdated');
    if (!dot) return;

    if (connected) {
        dot.className   = 'ds-dot connected';
        label.className = 'connected';
        label.textContent = 'Live';
        if (pushedAt) {
            try {
                const d = new Date(pushedAt);
                const isDateOnly = !pushedAt.includes('T');
                if (isDateOnly) {
                    time.textContent = ` · Pushed ${d.toLocaleDateString('en-US', { timeZone: 'UTC', month: 'short', day: 'numeric' })}`;
                } else {
                    time.textContent = ` · Pushed ${d.toLocaleString('en-US', {
                        timeZone: 'America/New_York',
                        month: 'short', day: 'numeric',
                        hour: 'numeric', minute: '2-digit', hour12: true
                    })} ET`;
                }
            } catch (e) {
                time.textContent = ` · ${pushedAt}`;
            }
        }
    } else {
        dot.className   = 'ds-dot offline';
        label.className = 'offline';
        label.textContent = 'Offline';
        if (time) time.textContent = ' · Cached data';
    }
}

// ── Main Init ──
async function init() {
    const timeEl = document.getElementById('lastUpdated');
    if (timeEl) timeEl.textContent = '–';

    let metrics, pulse, sectors, watchlist, vol, predictions, macro;
    let sheetsOk = false;
    let data_pushed_at = null;

    // Try Google Sheets first if configured, fallback to JSON files
    if (USE_SHEETS) {
        const sheetsData = await loadFromSheets();
        if (sheetsData) {
            ({ metrics, pulse, sectors, watchlist, vol, predictions, macro, data_pushed_at } = sheetsData);
            sheetsOk = true;
        }
    }

    // Fallback: load from individual JSON files (original behavior)
    if (!pulse) {
        [metrics, pulse, sectors, watchlist, vol, predictions, macro] = await Promise.all([
            loadJSON('metrics.json'), loadJSON('pulse.json'), loadJSON('sectors.json'),
            loadJSON('watchlist.json'), loadJSON('volatility.json'),
            loadJSON('predictions.json'), loadJSON('macro.json')
        ]);
    }

    renderMetrics(metrics);
    renderPulseChart(pulse);
    renderSectorChart(sectors);
    renderWatchlist(watchlist);
    renderVolChart(vol);
    renderPredictions(predictions);
    renderMacroNotes(macro);

    // New PaperPanda-inspired features
    renderSectorHeatmap(sectors);
    loadNewsOverview();
    buildTickerTape();

    // Pre-load insider data (used in both Overview mini and Insider tab)
    loadJSON('insider.json').then(d => {
        if (d && d.transactions) {
            insiderAll = d.transactions;
            renderInsiderMini(d.transactions);
        }
    });

    // Cache live prices for chart overlays
    if (watchlist?.stocks) {
        watchlist.stocks.forEach(s => {
            if (s.ticker) livePrices[s.ticker] = {
                price: s.price,
                change_pct: s.change_pct
            };
        });
    }

    // Merge server watchlist with localStorage defaults if first visit
    if (!localStorage.getItem(LS_KEY) && watchlist?.stocks) {
        const tickers = watchlist.stocks.map(s => s.ticker);
        saveWatchlist(tickers);
    }

    // Show data source status — green dot = Sheets connected, push time = when pipeline last wrote
    setDataStatus(sheetsOk, data_pushed_at);
}

// ══════════════════════════════════════════════════════════════════════════
// TICKER TAPE
// ══════════════════════════════════════════════════════════════════════════

async function fetchFinnhubQuote(symbol) {
    const sym = symbol.replace('-', ':'); // BTC-USD → BTC:USD for Finnhub? No - use as-is
    const url = `${FINNHUB_BASE}/quote?symbol=${encodeURIComponent(symbol)}&token=${FINNHUB_KEY}`;
    try {
        const resp = await fetch(url);
        if (!resp.ok) return null;
        return await resp.json();
    } catch { return null; }
}

async function buildTickerTape() {
    const track = document.getElementById('tickerTrack');
    if (!track) return;

    // Batch fetch quotes (with small delay between to stay inside rate limits)
    const symbols = TICKER_TAPE_SYMBOLS;
    const results = [];
    for (const sym of symbols) {
        const q = await fetchFinnhubQuote(sym);
        if (q && q.c) {
            const chgPct = q.dp != null ? q.dp : (q.c && q.pc ? ((q.c - q.pc) / q.pc * 100) : 0);
            results.push({ sym, price: q.c, chgPct });
        }
        await new Promise(r => setTimeout(r, 80)); // ~12/s, well under 60/min
    }

    if (results.length === 0) {
        track.innerHTML = '<span class="ticker-item">Market data unavailable</span>';
        return;
    }

    // Build ticker items (doubled for seamless loop)
    const makeItems = () => results.map(({ sym, price, chgPct }) => {
        const cls  = chgPct >= 0 ? 'up' : 'down';
        const sign = chgPct >= 0 ? '+' : '';
        const priceStr = price > 1000 ? price.toLocaleString('en-US', { maximumFractionDigits: 0 }) :
                         price > 100  ? price.toFixed(2) : price.toFixed(3);
        return `<span class="ticker-item">
            <span class="tk-sym">${sym}</span>
            <span class="tk-price">$${priceStr}</span>
            <span class="tk-chg ${cls}">${sign}${chgPct.toFixed(2)}%</span>
        </span><span class="tk-sep">|</span>`;
    }).join('');

    // Double the items so CSS animation loops seamlessly
    track.innerHTML = makeItems() + makeItems();
    // Set animation duration proportional to content length
    const duration = Math.max(40, results.length * 3.5);
    track.style.animationDuration = duration + 's';
}

// ══════════════════════════════════════════════════════════════════════════
// MARKET NEWS
// ══════════════════════════════════════════════════════════════════════════

function timeAgo(ts) {
    const diff = Math.floor(Date.now() / 1000) - ts;
    if (diff < 3600)   return Math.floor(diff / 60) + 'm ago';
    if (diff < 86400)  return Math.floor(diff / 3600) + 'h ago';
    return Math.floor(diff / 86400) + 'd ago';
}

function buildNewsCard(article, compact = false) {
    const ago = timeAgo(article.datetime);
    const src = article.source || 'News';
    const img = article.image ? `<img class="news-img" src="${article.image}" alt="" onerror="this.style.display='none'">` : '';
    if (compact) {
        return `<div class="news-item news-compact" onclick="window.open('${article.url}','_blank')">
            <div class="news-meta"><span class="news-source">${src}</span><span class="news-ago">${ago}</span></div>
            <div class="news-headline">${article.headline}</div>
        </div>`;
    }
    return `<div class="news-item" onclick="window.open('${article.url}','_blank')">
        ${img}
        <div class="news-content">
            <div class="news-meta"><span class="news-source">${src}</span><span class="news-ago">${ago}</span></div>
            <div class="news-headline">${article.headline}</div>
            <div class="news-summary">${(article.summary || '').slice(0, 140)}${(article.summary||'').length > 140 ? '…' : ''}</div>
        </div>
    </div>`;
}

async function fetchNews(category = 'general', limit = 15) {
    const url = `${FINNHUB_BASE}/news?category=${category}&token=${FINNHUB_KEY}`;
    try {
        const resp = await fetch(url);
        if (!resp.ok) return [];
        const data = await resp.json();
        return Array.isArray(data) ? data.slice(0, limit) : [];
    } catch { return []; }
}

async function loadNewsOverview() {
    const el = document.getElementById('newsListOverview');
    if (!el) return;
    const articles = await fetchNews('general', 8);
    if (!articles.length) {
        el.innerHTML = '<div class="news-loading">No news available.</div>';
        return;
    }
    el.innerHTML = articles.map(a => buildNewsCard(a, true)).join('');
}

async function loadNewsTab() {
    // General
    const genEl  = document.getElementById('newsListGeneral');
    const cryEl  = document.getElementById('newsListCrypto');
    const fxEl   = document.getElementById('newsListForex');

    const [general, crypto, forex] = await Promise.all([
        fetchNews('general', 20),
        fetchNews('crypto', 8),
        fetchNews('forex', 8),
    ]);

    if (genEl) genEl.innerHTML  = general.length  ? general.map(a => buildNewsCard(a, false)).join('')  : '<div class="news-loading">No news.</div>';
    if (cryEl) cryEl.innerHTML  = crypto.length   ? crypto.map(a => buildNewsCard(a, true)).join('')   : '<div class="news-loading">No crypto news.</div>';
    if (fxEl)  fxEl.innerHTML   = forex.length    ? forex.map(a => buildNewsCard(a, true)).join('')    : '<div class="news-loading">No forex news.</div>';
}

// ══════════════════════════════════════════════════════════════════════════
// SECTOR HEATMAP (Plotly Treemap)
// ══════════════════════════════════════════════════════════════════════════

function renderSectorHeatmap(sectorsData) {
    const el = document.getElementById('sectorHeatmap');
    if (!el || !sectorsData || !sectorsData.sectors) return;

    const sectors = sectorsData.sectors;
    const names   = Object.keys(sectors);
    const values  = names.map(n => sectors[n].change_pct || 0);

    // Color: red = negative, white = 0, green = positive
    const maxAbs = Math.max(...values.map(Math.abs), 1);
    const colorScale = [
        [0,    '#b91c1c'],
        [0.25, '#ef4444'],
        [0.45, '#fca5a5'],
        [0.5,  '#f5f5f5'],
        [0.55, '#86efac'],
        [0.75, '#22c55e'],
        [1,    '#15803d'],
    ];

    Plotly.newPlot(el, [{
        type:      'treemap',
        ids:       names,
        labels:    names.map((n, i) => `<b>${n}</b><br>${values[i] >= 0 ? '+' : ''}${values[i].toFixed(2)}%`),
        parents:   names.map(() => ''),
        values:    names.map(() => 1),
        customdata: values,
        marker: {
            colors:       values.map(v => (v + maxAbs) / (2 * maxAbs)), // normalize 0-1
            colorscale:   colorScale,
            cmin: 0, cmax: 1,
            line: { color: '#ffffff', width: 2 },
        },
        textfont: { family: "'IBM Plex Mono', monospace", size: 11, color: '#111111' },
        hovertemplate: '<b>%{id}</b><br>%{customdata:.2f}%<extra></extra>',
        tiling: { packing: 'squarify' },
    }], {
        paper_bgcolor: 'rgba(0,0,0,0)',
        margin: { l: 0, r: 0, t: 4, b: 4 },
        showlegend: false,
    }, { responsive: true, displayModeBar: false });
}

// ══════════════════════════════════════════════════════════════════════════
// INSIDER FLOW
// ══════════════════════════════════════════════════════════════════════════

let insiderAll = [];

function renderInsiderTable(filter) {
    const wrap = document.getElementById('insiderTableWrap');
    if (!wrap) return;

    let rows = insiderAll;
    if (filter === 'BUY')  rows = rows.filter(t => t.type === 'BUY');
    if (filter === 'SELL') rows = rows.filter(t => t.type === 'SELL');

    if (!rows.length) {
        wrap.innerHTML = '<div class="news-loading">No transactions match filter.</div>';
        return;
    }

    let html = `<table class="flow-table">
        <thead><tr>
            <th>Date</th><th>Ticker</th><th>Insider</th><th>Type</th>
            <th class="text-right">Shares</th><th class="text-right">Price</th><th class="text-right">Value</th>
        </tr></thead><tbody>`;

    rows.slice(0, 100).forEach(t => {
        const cls  = t.type === 'BUY' ? 'flow-buy' : 'flow-sell';
        const val  = t.value > 0 ? '$' + formatLarge(t.value) : '--';
        const prc  = t.price > 0 ? '$' + t.price.toFixed(2) : '--';
        const shr  = t.shares > 0 ? t.shares.toLocaleString() : '--';
        html += `<tr>
            <td class="flow-date">${t.date}</td>
            <td><span class="flow-ticker">${t.ticker}</span></td>
            <td class="flow-name">${t.name}</td>
            <td><span class="flow-type-badge ${cls}">${t.type}</span></td>
            <td class="text-right mono">${shr}</td>
            <td class="text-right mono">${prc}</td>
            <td class="text-right mono ${cls}">${val}</td>
        </tr>`;
    });
    html += '</tbody></table>';
    wrap.innerHTML = html;
}

function filterInsider(filter, btn) {
    document.querySelectorAll('#tab-insider .flow-btn').forEach(b => b.classList.remove('active'));
    if (btn) btn.classList.add('active');
    renderInsiderTable(filter);
}

function formatLarge(n) {
    if (n >= 1e9) return (n / 1e9).toFixed(1) + 'B';
    if (n >= 1e6) return (n / 1e6).toFixed(1) + 'M';
    if (n >= 1e3) return (n / 1e3).toFixed(0) + 'K';
    return n.toFixed(0);
}

function renderInsiderSummary(data) {
    const el = document.getElementById('insiderSummary');
    if (!el || !data) return;

    const txns     = data.transactions || [];
    const buys     = txns.filter(t => t.type === 'BUY');
    const sells    = txns.filter(t => t.type === 'SELL');
    const buyVal   = buys.reduce((s, t) => s + (t.value || 0), 0);
    const sellVal  = sells.reduce((s, t) => s + (t.value || 0), 0);

    // Ticker frequency
    const tickerCount = {};
    txns.forEach(t => { tickerCount[t.ticker] = (tickerCount[t.ticker] || 0) + 1; });
    const topTicker = Object.entries(tickerCount).sort((a, b) => b[1] - a[1])[0];

    el.innerHTML = `
        <div class="summary-grid">
            <div class="summary-stat">
                <div class="summary-num flow-buy">${buys.length}</div>
                <div class="summary-label">Purchases</div>
            </div>
            <div class="summary-stat">
                <div class="summary-num flow-sell">${sells.length}</div>
                <div class="summary-label">Sales</div>
            </div>
            <div class="summary-stat">
                <div class="summary-num">${txns.length}</div>
                <div class="summary-label">Total</div>
            </div>
        </div>
        <div class="summary-detail">
            <div class="summary-row"><span>Buy Volume</span><span class="flow-buy mono">$${formatLarge(buyVal)}</span></div>
            <div class="summary-row"><span>Sell Volume</span><span class="flow-sell mono">$${formatLarge(sellVal)}</span></div>
            <div class="summary-row"><span>Most Active</span><span class="mono">${topTicker ? topTicker[0] + ' (' + topTicker[1] + ')' : '--'}</span></div>
            <div class="summary-row"><span>Window</span><span>${data.from_date || '--'} → ${data.to_date || '--'}</span></div>
        </div>`;
}

function renderInsiderBigMoves(txns) {
    const el = document.getElementById('insiderBigMoves');
    if (!el) return;
    const sorted = [...txns].filter(t => t.value > 0).sort((a, b) => b.value - a.value).slice(0, 8);
    if (!sorted.length) { el.innerHTML = '<div class="news-loading">No data.</div>'; return; }
    el.innerHTML = sorted.map(t => {
        const cls = t.type === 'BUY' ? 'flow-buy' : 'flow-sell';
        return `<div class="big-move-item">
            <div class="big-move-left">
                <span class="flow-ticker">${t.ticker}</span>
                <span class="big-move-name">${t.name.split(' ').slice(0,2).join(' ')}</span>
            </div>
            <div class="big-move-right">
                <span class="flow-type-badge ${cls}">${t.type}</span>
                <span class="big-move-val mono ${cls}">$${formatLarge(t.value)}</span>
            </div>
        </div>`;
    }).join('');
}

async function renderInsiderTab() {
    const data = await loadJSON('insider.json');
    if (!data || !data.transactions) {
        document.getElementById('insiderTableWrap').innerHTML =
            '<div class="news-loading">No insider data yet. Run the pipeline first.</div>';
        return;
    }
    insiderAll = data.transactions;

    const updEl = document.getElementById('insiderUpdated');
    if (updEl && data.last_updated) {
        try {
            const d = new Date(data.last_updated);
            updEl.textContent = ' · Updated ' + d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        } catch {}
    }

    renderInsiderTable('ALL');
    renderInsiderSummary(data);
    renderInsiderBigMoves(data.transactions);

    // Also populate the mini sidebar on Overview tab
    renderInsiderMini(data.transactions);
}

function renderInsiderMini(txns) {
    const el = document.getElementById('insiderMiniList');
    if (!el) return;
    const top = txns.slice(0, 5);
    if (!top.length) { el.innerHTML = '<div class="news-loading">No data.</div>'; return; }
    el.innerHTML = top.map(t => {
        const cls = t.type === 'BUY' ? 'flow-buy' : 'flow-sell';
        return `<div class="insider-mini-item">
            <span class="flow-ticker">${t.ticker}</span>
            <span class="flow-type-badge ${cls}">${t.type}</span>
            <span class="insider-mini-name">${t.name.split(' ').slice(0,2).join(' ')}</span>
            <span class="mono ${cls}" style="margin-left:auto;font-size:0.75rem;">${t.value > 0 ? '$' + formatLarge(t.value) : ''}</span>
        </div>`;
    }).join('');
}

// ══════════════════════════════════════════════════════════════════════════
// CONGRESS WATCH
// ══════════════════════════════════════════════════════════════════════════

let congressAll = [];
let congressFilters = { chamber: 'ALL', search: '' };

function applyCongressFilters() {
    let rows = congressAll;
    const { chamber, search } = congressFilters;

    if (chamber !== 'ALL') {
        if (chamber === 'Purchase' || chamber === 'Sale') {
            rows = rows.filter(t => t.type.toLowerCase().includes(chamber.toLowerCase()));
        } else {
            rows = rows.filter(t => t.chamber === chamber);
        }
    }
    if (search) {
        const q = search.toLowerCase();
        rows = rows.filter(t =>
            t.ticker.toLowerCase().includes(q) ||
            t.member.toLowerCase().includes(q) ||
            t.type.toLowerCase().includes(q) ||
            t.asset.toLowerCase().includes(q)
        );
    }
    return rows;
}

function renderCongressTable() {
    const wrap = document.getElementById('congressTableWrap');
    if (!wrap) return;
    const rows = applyCongressFilters();

    if (!rows.length) {
        wrap.innerHTML = '<div class="news-loading">No transactions match filter.</div>';
        return;
    }

    let html = `<table class="flow-table">
        <thead><tr>
            <th>Date</th><th>Chamber</th><th>Member</th><th>Ticker</th>
            <th>Type</th><th>Amount</th><th>Asset</th>
        </tr></thead><tbody>`;

    rows.slice(0, 150).forEach(t => {
        const isPurchase = t.type.toLowerCase().includes('purchase');
        const isSale     = t.type.toLowerCase().includes('sale');
        const cls = isPurchase ? 'flow-buy' : isSale ? 'flow-sell' : '';
        const chamberBadge = t.chamber === 'Senate' ? 'congress-senate' : 'congress-house';
        html += `<tr>
            <td class="flow-date">${t.date}</td>
            <td><span class="congress-badge ${chamberBadge}">${t.chamber}</span></td>
            <td class="flow-name">${t.member}</td>
            <td><span class="flow-ticker">${t.ticker}</span></td>
            <td><span class="flow-type-badge ${cls}">${t.type}</span></td>
            <td class="mono" style="font-size:0.78rem;">${t.amount}</td>
            <td style="color:var(--ink-muted);font-size:0.75rem;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${t.asset}</td>
        </tr>`;
    });
    html += '</tbody></table>';
    wrap.innerHTML = html;
}

function filterCongressChamber(val, btn) {
    document.querySelectorAll('#tab-congress .flow-btn').forEach(b => b.classList.remove('active'));
    if (btn) btn.classList.add('active');
    congressFilters.chamber = val;
    renderCongressTable();
}

function filterCongress(q) {
    congressFilters.search = q;
    renderCongressTable();
}

function renderCongressSummary(data) {
    const el = document.getElementById('congressSummary');
    if (!el || !data) return;

    const txns     = data.transactions || [];
    const house    = txns.filter(t => t.chamber === 'House').length;
    const senate   = txns.filter(t => t.chamber === 'Senate').length;
    const buys     = txns.filter(t => t.type.toLowerCase().includes('purchase')).length;
    const sells    = txns.filter(t => t.type.toLowerCase().includes('sale')).length;

    el.innerHTML = `
        <div class="summary-grid">
            <div class="summary-stat">
                <div class="summary-num">${house}</div>
                <div class="summary-label">House</div>
            </div>
            <div class="summary-stat">
                <div class="summary-num">${senate}</div>
                <div class="summary-label">Senate</div>
            </div>
            <div class="summary-stat">
                <div class="summary-num">${txns.length}</div>
                <div class="summary-label">Total</div>
            </div>
        </div>
        <div class="summary-detail">
            <div class="summary-row"><span>Purchases</span><span class="flow-buy mono">${buys}</span></div>
            <div class="summary-row"><span>Sales</span><span class="flow-sell mono">${sells}</span></div>
            <div class="summary-row"><span>Window</span><span>${data.cutoff_date || '--'} → now</span></div>
        </div>`;
}

function renderTopCongressTickers(txns) {
    const el = document.getElementById('congressTopTickers');
    if (!el) return;
    const count = {};
    txns.forEach(t => { count[t.ticker] = (count[t.ticker] || 0) + 1; });
    const sorted = Object.entries(count).sort((a, b) => b[1] - a[1]).slice(0, 12);
    if (!sorted.length) { el.innerHTML = '<div class="news-loading">No data.</div>'; return; }
    el.innerHTML = sorted.map(([tk, cnt]) => `
        <div class="top-ticker-row">
            <span class="flow-ticker">${tk}</span>
            <div class="top-ticker-bar-wrap">
                <div class="top-ticker-bar" style="width:${Math.round(cnt / sorted[0][1] * 100)}%"></div>
            </div>
            <span class="top-ticker-count mono">${cnt}</span>
        </div>`).join('');
}

async function renderCongressTab() {
    const data = await loadJSON('congress.json');
    if (!data || !data.transactions) {
        document.getElementById('congressTableWrap').innerHTML =
            '<div class="news-loading">No congressional data yet. Run the pipeline first.</div>';
        return;
    }
    congressAll = data.transactions;

    const updEl = document.getElementById('congressUpdated');
    if (updEl && data.last_updated) {
        try {
            const d = new Date(data.last_updated);
            updEl.textContent = ' · Updated ' + d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        } catch {}
    }

    renderCongressTable();
    renderCongressSummary(data);
    renderTopCongressTickers(data.transactions);
}

document.addEventListener('DOMContentLoaded', init);
